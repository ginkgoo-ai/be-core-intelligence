...
from src.model.workflow_schemas import (
    ...
    FormProcessResult, FormActionModel
)
from src.business.langgraph_form_processor import LangGraphFormProcessor
from src.model.profile_schema import Profile

class WorkflowService:
    """Main workflow service implementing 12-step visa application process"""
    
    # Default 12 steps of visa application workflow (fallback if no definition provided)
    DEFAULT_VISA_WORKFLOW_STEPS = [
        {"key": "applicant_setup", "name": "1. Applicant & Application Setup", "order": 1},
        {"key": "personal_details", "name": "2. Personal Details", "order": 2},
        {"key": "contact_address", "name": "3. Contact & Address", "order": 3},
        {"key": "family_details", "name": "4. Family Details", "order": 4},
        {"key": "travel_history", "name": "5. Travel History", "order": 5},
        {"key": "immigration_history", "name": "6. Immigration History", "order": 6},
        {"key": "employment_sponsor", "name": "7. Employment & Sponsor", "order": 7},
        {"key": "english_language", "name": "8. English Language", "order": 8},
        {"key": "financial_requirements", "name": "9. Financial Requirements", "order": 9},
        {"key": "security_character", "name": "10. Security & Character", "order": 10},
        {"key": "additional_information", "name": "11. Additional Information", "order": 11},
        {"key": "application_declaration", "name": "12. Application & Declaration", "order": 12}
    ]
        ...
        try:
            ...
            if payload.workflow_definition_id:
                ...
                if workflow_def and workflow_def.step_definitions:
                    ...
                    print(f"⚠️ 工作流定义未找到或无步骤配置，使用默认12步配置")
            else:
                ...
                workflow_def = self.definition_repo.create_definition(
                    ...
                    description="Default 12-step visa application process",
                    steps_schema=step_definitions
                )
            ...
            print(f"✅ 创建了 {len(steps)} 个步骤实例")
            
            # Set first step as active (based on order)
            if steps:
...
class StepService:
    ...
    def process_form_for_step(self, workflow_id: str, step_key: str, form_html: str, profile_data: dict) -> FormProcessResult:
        ...
        try:
            ...
            session_id = f"{workflow_id}_{step_key}"
            
            # Use LangGraph form processor to analyze and generate actions
            result = self.form_processor.process_form(workflow_id, step_key, form_html, profile_data)
            ...
            if result["success"]:
                ...
                return FormProcessResult(
                    ...
                    # 使用新的简化格式
                    questions=[],  # 不再分离返回，data已经包含了合并格式
                    answers=[],   # 不再分离返回，data已经包含了合并格式
    ...
    def _advance_workflow_step(self, workflow_id: str, current_step_key: str):
        ...
        next_step_info = self._get_next_step(workflow_id, current_step_key)
    ...
    def _get_next_step(self, workflow_id: str, current_step_key: str) -> Dict[str, Any]:
        """Get next step information from database step instances"""
        ...
        # Sort steps by order and find the next one
